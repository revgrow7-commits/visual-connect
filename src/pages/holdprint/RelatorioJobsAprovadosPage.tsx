...
function useApprovedJobs() {
  return useQuery<ApprovedJob[]>({
    queryKey: ["holdprint-approved-jobs-report"],
    queryFn: async () => {
      // Fetch jobs from both units via cs-holdprint-data
      ...
      // Filter approved jobs (not cancelled, has financial approval indicators)
      const approved = allJobs
        .filter((j) => {
          const status = String(j.status || "").toLowerCase();
          const chargeStatus = String(j.jobChargeStatus || "").toLowerCase();
          const invoiceStatus = String(j.jobInvoiceStatus || "").toLowerCase();
          const isFinalized = Boolean(j.isFinalized);
          // Include jobs that are active or finalized (exclude cancelled)
  ...
  return (
    <div className="space-y-6">
      ...
      {isLoading ? (
        ...
        <Card><CardContent className="py-8 text-center text-destructive">Erro ao carregar dados: {(error as Error).message}</CardContent></Card>
      ) : (
        <Card>
